  ...
  return (
    <>
      ...
      <div className="sm:hidden space-y-3">
        {clientes.map((c) => (
          <div key={c.id_cliente} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                  CI/NIT: {c.ci_nit || 'S/N'}
                  {c.empresa && <span className="block">{c.empresa}</span>}
                </p>
              </div>
              {puede('activar', 'clientes') ? (
                <button
            ...
            {(c.telefono || c.correo) && (
              <div className="mt-3 pt-3 border-t border-zinc-100 dark:border-zinc-800 text-sm text-zinc-600 dark:text-zinc-400">
                <p>{c.telefono || '—'}</p>
                {c.correo && <p className="truncate">{c.correo}</p>}
              </div>
            )}
