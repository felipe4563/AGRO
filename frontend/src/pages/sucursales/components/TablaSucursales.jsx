  ...
  if (!sucursales || sucursales.length === 0) {
    return (
      <div className="p-8 text-center text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <p className="text-lg">No hay sucursales registradas.</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {sucursales.map((s) => (
              <tr key={s.id_sucursal} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">
                  <p className="font-medium text-zinc-800 dark:text-zinc-200">{s.ciudad}</p>
                  <p className="text-xs truncate max-w-[200px]" title={s.direccion}>{s.direccion}</p>
                </td>
                <td className="px-4 py-3 hidden md:table-cell">
                  <p className="text-xs text-zinc-600 dark:text-zinc-400">{s.correo || '—'}</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-500">{s.telefono || '—'}</p>
                </td>
                <td className="px-4 py-3 text-center">
                  {puede('activar', 'sucursales') ? (
                    <button
                      onClick={() => onToggleActivo(s)}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${
                        s.activo ? 'bg-emerald-500' : 'bg-zinc-300 dark:bg-zinc-600'
                      }`}
                      title={s.activo ? 'Desactivar sucursal' : 'Activar sucursal'}
                    >
                      <span
                        className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${
                          s.activo ? 'translate-x-4' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  ) : (
                    <span
                      className={`inline-block h-3.5 w-3.5 rounded-full ${
                        s.activo ? 'bg-emerald-500' : 'bg-zinc-400'
                      }`}
